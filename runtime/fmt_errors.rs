...
fn stack_frame_is_ext(frame: &deno_core::error::JsStackFrame) -> bool {
    frame
        .file_name
        .as_ref()
        .map(|file_name| file_name.starts_with("ext:") || file_name.starts_with("node:"))
        .unwrap_or(false)
}
    ...
    if let Some(msg) = &e.message {
            ...
            || msg.contains("require is not defined")
        {
            ...
            return vec![
                ...
                FixSuggestion::hint("Run again with `--unstable-bundle` flag to enable this API."),
            ];
            ...
            && msg.contains(".node'")
        {
            ...
            return vec![
                FixSuggestion::info(cstr!("<u>document</> global is not available in Deno.")),
                FixSuggestion::hint_multiline(&[
                    cstr!(
...
pub fn format_js_error(js_error: &JsError, initial_cwd: Option<&Url>) -> String {
