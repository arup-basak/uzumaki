...
pub fn maybe_transpile_source(
    ...
    let media_type = if name_str.starts_with("node:") || name_str == "uzumaki" {
        MediaType::TypeScript
    } else {
        MediaType::from_path(Path::new(name_str))
    };

    let specifier = if name_str == "uzumaki" {
        deno_core::url::Url::parse("ext:uzumaki").unwrap()
    } else {
        deno_core::url::Url::parse(name_str).unwrap()
    };
